      ...
      {isLoading ? (
        ...
        <div className="space-y-8 pb-10">
          {grades.map((grade) => (
            <div
              key={grade.id}
              className="mx-auto max-w-7xl px-4 py-8 sm:py-16 lg:px-10 rounded-3xl bg-lightgrey grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5"
            >
              ...
              {(grade.subjects ?? []).map((subject: any) => (
                <Card key={subject.id} className="flex flex-col m-2 bg-white">
                  <CardContent className="flex-grow p-6">
                    <div className="h-16 flex items-center justify-center">
                    ...
                    <p className="text-center text-muted-foreground">
                      {subject.description ||
                        `Explore lessons and resources for ${subject.title}.`}
                    </p>
                  </CardContent>
                  <CardFooter className="p-6 pt-0">
                    <button
                      ...
                      onClick={() => onHandleSubjectClick(subject.id)}
                    >
                      Start Learning
                    </button>
                  </CardFooter>
