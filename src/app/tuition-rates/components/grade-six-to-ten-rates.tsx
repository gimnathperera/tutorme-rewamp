...
const GradeSixToNineRates = () => {
  ...
  if (!SecondaryLevelInfo) {
    return <div>Error: Grade 6 to 9 Level Tuition details not found.</div>;
  }
  return (
    <div>
      <div className="space-y-4 m-10">
        <div className="flex space-y-4 justify-center items-center flex-col mb-10">
          <p className="text-[#FCA627]  text-[18px] font-bold">
            {SecondaryLevelInfo.subTitle}
          </p>
          <h2 className="text-3xl font-semibold">{SecondaryLevelInfo.title}</h2>
          <p className="text-gray-600">{SecondaryLevelInfo.description}</p>
        </div>
