...
const LocaleSwitcher = ({ compact = false, onDark = false }: LocaleSwitcherProps) => {
    ...
    <Select value={locale} onValueChange={(val) => handleChange(val as Locale)}>
      <SelectTrigger
        className={
          compact
            ? compactClass
            : "h-10 w-auto gap-1 rounded-full border border-primary-200 bg-white px-4 py-2 text-sm font-medium text-primary-700 shadow-none hover:bg-primary-100 hover:border-primary-400 focus:ring-1 focus:ring-primary-300 transition-colors duration-150 [&>span]:flex [&>span]:items-center"
        }
